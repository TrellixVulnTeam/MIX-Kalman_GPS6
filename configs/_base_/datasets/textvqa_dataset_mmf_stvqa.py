...
data_root = '/home/datasets/mix_data/mixk/'
...
stvqa_path = 'data/datasets/stvqa/defaults/'

# train_datasets = ['train_en']  # train_en
train_datasets = ['stvqa']
...
annotations = dict(
    ...
    # train_en=data_root + annotation_path + 'imdb_train_ocr_en_phoc_feature.npy',
    ...
    # val_en=data_root + annotation_path + 'imdb_val_ocr_en_phoc_feature.npy',
    ...
    stvqa=data_root + stvqa_path + 'annotations/imdb_subtrain.npy')
...
textvqa_reader_train_cfg = dict(
    ...
    mix_features=dict(
        ...
        stvqa=data_root + stvqa_path + 'features/detectron.lmdb',
    ),
    ...
    mix_ocr_features=dict(
        ...
        stvqa=data_root + stvqa_path + 'ocr_features/ocr_en_frcn_features.lmdb'),
...
textvqa_info_cpler_cfg = dict(
    ...
    text_processor=dict(
        type='bert_tokenizer',
        tokenizer_config=dict(type='bert-base-uncased', params=dict(do_lower_case=True)),
        max_seq_length=20),
    answer_processor=dict(
        ...
        max_copy_steps=12,
        num_answers=10),
    copy_processor=dict(type='copy', max_length=100),
    phoc_processor=dict(type='phoc', max_length=50),
    context_processor=dict(type='fasttext', max_length=50, model_file=data_root + 'fasttext/wiki.en.bin'),
    ocr_token_processor=dict(type='simple_word'),
    bbox_processor=dict(type='bbox', max_length=50),
...
post_processor = dict(
    ...
    dataset_converters=[dict(type='TextVQADatasetConvert')])
