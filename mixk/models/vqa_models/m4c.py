...
from ..builder import VQA_MODELS, build_encoder, build_head
from .base_model import BaseModel
# from mixk.data.infocomp.textvqa_infocpler import TextVQAAnswerProcessor
...
class M4C(BaseModel):
    ...
    def __init__(self, m4c_config):
        super().__init__()
        self.config = m4c_config
    ...
    def _build_obj_encoding(self):
        ...
        self.obj_faster_rcnn_fc7 = build_encoder(self.config.obj)
    ...
    def _build_ocr_encoding(self):
        ...
        self.ocr_faster_rcnn_fc7 = build_encoder(self.config.ocr)
    ...
    def _build_output(self):
        ...
        self.classifier = build_head(self.config.head)

        # self.answer_processor = registry.get(self._datasets[0] + "_answer_processor")
        vocab_file = self.config.answer_processor.vocab_file
        self.answer_processor = TextVQAAnswerProcessor(vocab_file)

    def forward_train(self, data, **kwargs):
        # fwd_results holds intermediate forward pass results
        # TODO possibly replace it with another sample list
        data = self.cpu_to_cuda(data, 'cuda')
        data = ItemFeature(data)
        fwd_results = {}
        ...
        self._forward_mmt_and_output(data, fwd_results)
    ...
    @staticmethod
    def cpu_to_cuda(data, device, non_blocking=True):
    ...
    def _forward_txt_encoding(self, sample_list, fwd_results):
        ...
        fwd_results['train_loss_mask'] = sample_list['train_loss_mask']
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        obj_nums = sample_list.image_info_0_max_features
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_nums = sample_list.context_info_0_max_features
    ...
    def _forward_mmt_and_output(self, sample_list, fwd_results):
        if self.training:
            fwd_results['prev_inds'] = sample_list['train_prev_inds'].clone().cuda()
