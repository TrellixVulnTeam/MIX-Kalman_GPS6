...
from mixk.data.builder import DATASETS
import logging
from mixk.data.utils.stream import ItemFeature
...
class VcrTxtTokLmdb(TxtTokLmdb):
    ...
    def __init__(self, db_dir, max_txt_len=120, task='qa,qar'):
        ...
        if max_txt_len == -1:
            ...
            self.id2len = {
                id_: len_
                for id_, len_ in json.load(open(f'{db_dir}/id2len_{id2len_task}.json')).items() if len_ <= max_txt_len
            }
...
class VcrDetectFeatTxtTokDataset(DetectFeatTxtTokDataset):
    ...
    def __init__(self, txt_db, img_db_gt=None, img_db=None):
        ...
        if self.img_db and self.img_db_gt:
            self.lens = [
                tl + self.img_db_gt.name2nbb[txt2img[id_][0]] + self.img_db.name2nbb[txt2img[id_][1]]
                for tl, id_ in zip(txt_lens, self.ids)
            ]
        elif self.img_db:
            self.lens = [tl + self.img_db.name2nbb[txt2img[id_][1]] for tl, id_ in zip(txt_lens, self.ids)]
        else:
            self.lens = [tl + self.img_db_gt.name2nbb[txt2img[id_][0]] for tl, id_ in zip(txt_lens, self.ids)]
...
def load_img_feat(db_list, all_img_dbs, opts):
    ...
    for d in db_:
        if 'gt' in d:
            ...
            db_path = d
    if gt_db_path != '':
        img_db_gt = DetectFeatLmdb(gt_db_path, -1, opts.max_bb, opts.min_bb, 100, opts.compressed_db)
    ...
    def __init__(self, **kwargs):
        ...
        if train_or_val:  # train
            ...
            for txt_path, img_path in zip(opts.train_txt_dbs, opts.train_img_dbs):
                ...
                train_datasets.append(VcrTrainDataset(qa_txt_db, img_db_gt=img_db_gt, img_db=img_db))
                train_datasets.append(VcrTrainDataset(qar_txt_db, img_db_gt=img_db_gt, img_db=img_db))
...
class VcrTrainDataset(VcrDetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        for index, input_ids_a in enumerate(input_ids_for_choices):
            ...
            txt_type_ids = [0] + type_ids_q + [type_id_for_choice] * (len(input_ids_a) + 2)
            attn_masks = torch.ones(len(input_ids) + num_bb, dtype=torch.long)
            input_ids = torch.tensor(input_ids)
            txt_type_ids = torch.tensor(txt_type_ids)

            outs.append((input_ids, txt_type_ids, img_feat, img_pos_feat, attn_masks, target))

        return tuple(outs)


def vcr_collate(inputs):
    (input_ids, txt_type_ids, img_feats, img_pos_feats, attn_masks, targets) = map(list, unzip(concat(inputs)))
...
class VcrEvalDataset(VcrDetectFeatTxtTokDataset):
    ...
    def _get_input_ids(self, txt_dump):
        ...
        for index, input_ids_a in enumerate(input_ids_as):
            curr_input_ids_qa = \
                [self.txt_db.cls_] + copy.deepcopy(input_ids_q) + [self.txt_db.sep] + input_ids_a + [self.txt_db.sep]
        ...
        for index, input_ids_a in enumerate(input_ids_as):
            ...
            if (self.split == 'val' and index == txt_dump['qa_target']) or self.split == 'test':
                for input_ids_r in input_ids_rs:
                    curr_input_ids_qar = copy.deepcopy(curr_input_ids_qa) + input_ids_r + [self.txt_db.sep]
                    curr_type_ids_qar = copy.deepcopy(curr_type_ids_qa) + [3] * (len(input_ids_r) + 2)
...
def vcr_eval_collate(inputs):
    (input_ids, txt_type_ids, img_feats, img_pos_feats,
     attn_masks) = map(list, unzip(concat(outs for outs, _, _, _ in inputs)))
